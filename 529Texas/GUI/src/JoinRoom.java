...
public class JoinRoom extends JFrame implements MouseListener {
	String token;
	HallUI parent;
	String roomId;
	...
	public JoinRoom(String token, HallUI parent, String roomId) {
...
//实例化
		...
		Title_img = new JLabel("加入房间" + roomId);
...
//事件区域
	...
	public void mousePressed(MouseEvent e) {// 点击后
		if (e.getSource() == small_mig) {
			...
			System.out.println("JoinRoom：加入房间");
			System.out.println("JoinRoom: passWord is" + passWord);
			//hallController.handler_search_room(token, roomId, passWord);
			JSONObject obj = new JSONObject();
			obj.put("token", token);
			obj.put("roomId", this.roomId);
			obj.put("password", passWord);
			System.out.println("JoinRoom: roomId is" + roomId);
			String res = TexasHttpUtils.request("localhost:8888", "hall", "enter", obj.toString());
			//System.out.println(res);
			TexasHttpUtils.updateByJEvent(res, JoinRoom.this.parent);
			dispose();
