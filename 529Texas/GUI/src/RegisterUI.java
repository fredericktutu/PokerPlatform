...
public class RegisterUI extends JFrame implements MouseListener {
	...
	JLabel small_btn, close_btn, lie1, lie2, lie3, lie4;// 暗色块|线
	JTextField user;// 账号
	JTextField user_name;//昵称
	JPasswordField pass;// 密码
	JPasswordField confirmPass;// 密码
	JPanel bgcolor;// 白
	JLabel account_img, password_img, ku1, ku2, gou1, gou2,name_img,confirm_img;// 缩略图
	JLabel auto_lgin, Remember_pwd, find_pwd, sign_up, login;// 自动登录，记住密码，找回密码，注册账号，登录
...
//位置
		...
		login.setBounds(206, 290, 80, 20);
		submit.setBounds(100, 285, 242, 35);
//属性
		...
		user_name.setText("昵称");
		user_name.setOpaque(false);// 透明背景
		user_name.setBorder(null);// 去掉边框
		user_name.setFont(new Font("微软雅黑", Font.PLAIN, 16));// 框内文字样式
		...
		confirmPass.setFont(new Font("微软雅黑", Font.PLAIN, 16));// 框内文字样式
		confirmPass.setBorder(null);// 去掉边框
		confirmPass.setOpaque(false);// 透明背景
		confirmPass.setForeground(Color.gray);
		confirmPass.setText("确认密码");
		confirmPass.setEchoChar((char) 0);// 让密码显示出来
...
//事件区域
		...
		confirmPass.addMouseListener(this);
		...
		user.addFocusListener(new FocusListener() {
			...
			public void focusGained(FocusEvent e) {// 得到焦点
				...
				lie3.setIcon(new javax.swing.ImageIcon("素材//直线3.png"));
				a = 1;
				c = 1;
				b = 0;
		...
		confirmPass.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent e) {// 失去焦点
				lie4.setIcon(new javax.swing.ImageIcon("素材//直线2.png"));// 失去焦点换图片
				ImageIcon iconOrigin = new ImageIcon("素材//确认密码1.png");
				Image image = iconOrigin.getImage();
				Image newImage = image.getScaledInstance(20, 20, java.awt.Image.SCALE_SMOOTH);
				confirm_img.setIcon(new javax.swing.ImageIcon(newImage));
				d = 0;
			...
			public void focusGained(FocusEvent e) {// 得到焦点
				ImageIcon iconOrigin = new ImageIcon("素材//确认密码.png");
				Image image = iconOrigin.getImage();
				Image newImage = image.getScaledInstance(20, 20, java.awt.Image.SCALE_SMOOTH);
				confirm_img.setIcon(new javax.swing.ImageIcon(newImage));
				lie4.setIcon(new javax.swing.ImageIcon("素材//直线3.png"));
				b = 1;
				a = 0;
				d = 1;
				confirmPass.setForeground(Color.black);
				confirmPass.setEchoChar('*');// 让用户输入看不见
				if (confirmPass.getText().equals("确认密码")) {
					confirmPass.setText("");
				} else {
					confirmPass.setText(confirmPass.getText());
				}
			}
		});
		...
		getContentPane().add(confirm_img);
	...
	public void mousePressed(MouseEvent e) {// 点击后
		if (e.getSource() == small_mig) {
			...
			login.setFont(new Font("微软雅黑", 0, 14));
			dispose();
			
			String names = user_name.getText();//name 
			String users = user.getText();//id
			...
			try{
				...
				*/
				...
				obj.put("username", names);
				obj.put("id", users);
				obj.put("password", password);

				String res = TexasHttpUtils.request("localhost:8888", "server", "register", obj.toString());
				if(res.startsWith("t")) {
					succeedToRegister = true;
				}
				
			} catch(Exception ee) {
				JOptionPane.showMessageDialog(null, "注册失败，请检查网络状况！");
			}
			...
			if (succeedToRegister) {
				...
				JOptionPane.showMessageDialog(null, "注册失败！账号已被注册");
				//RegisterUI.this.dispose();
				new RegisterUI();
			}
		}
	...
	public void mouseEntered(MouseEvent e) {// 悬停
		if (e.getSource() == small_mig) {
			...
			if (a == 0 && c == 0) {
				lie3.setIcon(new javax.swing.ImageIcon("素材//直线4.png"));
			}
		...
		} else if (e.getSource() == confirmPass) {
			if (b == 0 && d == 0) {
				lie4.setIcon(new javax.swing.ImageIcon("素材//直线4.png"));
			}
	...
	public void mouseExited(MouseEvent e) {// 悬停后
		if (e.getSource() == small_mig) {
			...
			if (b == 0) {
				lie4.setIcon(new javax.swing.ImageIcon("素材//直线2.png"));
			}
		}else if (e.getSource() == find_pwd) {
			find_pwd.setForeground(new Color(170, 170, 170));
		} else if (e.getSource() == sign_up) {
