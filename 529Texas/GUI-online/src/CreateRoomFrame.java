...
public class CreateRoomFrame extends JFrame implements MouseListener {
	...
	public CreateRoomFrame(String token, HallUI parent) {
		this.parent = parent;
...
//事件区域
		...
		submit.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				System.out.println("CreateRoomFrame: toString()" + option.roomPassport);
				CreateRoomFrame.this.dispose();

				JSONObject obj = new JSONObject();
				obj.put("token", token);
				JSONObject opt = new JSONObject();
				opt.put("difType", option.difficultyType);
				opt.put("personType", option.personType);
				opt.put("modeType", option.modeType);
				opt.put("passWord", option.roomPassport);
				opt.put("gameType", option.gameType);
				opt.put("levelType", option.levelType);
				obj.put("option", opt);
				System.out.println("CreateRoomFrame: ");
				System.out.println(obj);
	
				String res = TexasHttpUtils.request("localhost:8888", "hall", "create", obj.toString());
				TexasHttpUtils.updateByJEvent(res, CreateRoomFrame.this.parent);
				
				//JSONObject hejson = (JSONObject)parser.parse(res);
