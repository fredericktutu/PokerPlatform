		...
		if (e.getSource() == small_mig) {
			...
			try{
				...
				*/
				...
				String res = TexasHttpUtils.request("localhost:8888", "server", "login", obj.toString());
				

				if(!res.equals("") && !res.equals("!")) {
					int index = res.indexOf('&');
					token = res.substring(0, index);
					name = res.substring(index + 1);
				} else {
					token = res;
				}
			} catch(Exception ee) {
				JOptionPane.showMessageDialog(null, "登陆失败，请检查网络状况！");
				new LoginUI();
				return;
			}

			if (!token.equals("") && !token.equals("!")) {
				JOptionPane.showMessageDialog(null, "您好！"+name +",欢迎使用智能扑克游戏平台！");
				try {
					HallUI window = new HallUI(token, name);
				...
				// 错误密码或账号
				JOptionPane.showMessageDialog(null, "登陆失败，错误的账号或密码");
				new LoginUI();
			} else if(token.equals("!")) {
				JOptionPane.showMessageDialog(null, "登陆失败，您已在另一客户端登录");
				new LoginUI();
			}
		}
	}
